...
class OccupancyGrid2d(object):
    ...
    def SensorCallback(self, msg):
        ...
        for idx, r in enumerate(msg.ranges):
            ...
            ff_angle = msg.angle_min +yaw + idx*msg.angle_increment 
            ...
            #r_res = self._y_res/np.sin(ff_angle)
            r_res = np.mean([[self._x_res],[self._y_res]])

            for i in np.arange(0,r,r_res):

                   print i
                   x = (r-i)*np.cos(ff_angle) +sensor_x
                   y = (r-i)*np.sin(ff_angle) +sensor_y
                   Vox_id_x, vox_id_y = self.PointToVoxel(x,y)
                  
                   if i == 0:
                      self._map[Vox_id_x,vox_id_y]= self._map[Vox_id_x,vox_id_y] + self._occupied_update
                        
                      if self._map[Vox_id_x,vox_id_y] > self._occupied_threshold:
                            self._map[Vox_id_x,vox_id_y] = self._occupied_threshold

                   else:

                       self._map[Vox_id_x,vox_id_y]= self._map[Vox_id_x,vox_id_y] + self._free_update

                       if self._map[Vox_id_x,vox_id_y] < self._free_threshold:
                          self._map[Vox_id_x,vox_id_y]  = self._free_threshold

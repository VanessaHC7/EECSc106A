...
def threshold_segment_naive(gray_img, lower_thresh, upper_thresh):
    ...
    # #mask[gray_img_copy>lower_thresh] = 255
    # #mask[gray_img_copy<upper_thresh] = 255
    
   

    gray_img_copy[(gray_img_copy>lower_thresh) & (gray_img_copy<upper_thresh)] = 1
    print(np.min(gray_img_copy))
    gray_img_copy[(gray_img_copy != 1)] = 0

    print(gray_img_copy)
    print(gray_img)
    print(np.max(gray_img_copy), np.min(gray_img_copy))
    print(lower_thresh)
    print (upper_thresh)
...
def edge_detect_naive(gray_img):
    ...
    G_x = np.array(G_x,dtype = np.int64)
    G_y = np.array(G_y,dtype = np.int64)
...
def cluster_segment(img, n_clusters, random_state=0):
    ...
    clusters = do_kmeans(img_r, 3)

    # reshape this clustered image to the original downsampled image (img_d) width and height 
    cluster_img = clusters.reshape(original_shape[0],original_shape[1])
...
def segment_image(img): 
    ...
    binary = threshold_segment_naive(to_grayscale(img),0,100 ).astype(np.uint8)
...
if __name__ == '__main__':
    ...
    test_thresh_naive(test_img, 0, 100)

...
class OccupancyGrid2d(object):
    ...
    def SensorCallback(self, msg):
        ...
        for idx, r in enumerate(msg.ranges):
            ...
            angle_odom = angle_robot + yaw
            endpoint_x = sensor_x + np.cos(angle_odom)*r  
            endpoint_y = sensor_y + np.sin(angle_odom)*r 

            # get voxel of endpoint and increase log-odds ratio
            endpoint_x_voxel, endpoint_y_voxel = self.PointToVoxel(endpoint_x, endpoint_y)
            self._map[endpoint_x_voxel, endpoint_y_voxel] += self._occupied_update
            if self._map[endpoint_x_voxel, endpoint_y_voxel] > self._occupied_threshold:
                self._map[endpoint_x_voxel, endpoint_y_voxel] = self._occupied_threshold

            # walk "backwards" from endpoint to sensor_point by decrementing from r
            curr_r = r - math.sqrt(self._x_res**2 + self._y_res**2)
            currpoint_x = sensor_x + np.cos(angle_odom)*curr_r
            currpoint_y = sensor_y + np.sin(angle_odom)*curr_r
            while abs(currpoint_x - sensor_x) > self._x_res or abs(currpoint_y - sensor_y) > self._y_res:
                currpoint_x_voxel, currpoint_y_voxel = self.PointToVoxel(currpoint_x, currpoint_y)
                self._map[currpoint_x_voxel, currpoint_y_voxel] += self._free_update
                #print(self._map[currpoint_x_voxel, currpoint_y_voxel])
                if self._map[currpoint_x_voxel, currpoint_y_voxel] < self._free_threshold:
                    self._map[currpoint_x_voxel, currpoint_y_voxel] = self._free_threshold

                curr_r -= math.sqrt(self._x_res**2 + self._y_res**2)
                currpoint_x = sensor_x + np.cos(angle_odom)*curr_r
                currpoint_y = sensor_y + np.sin(angle_odom)*curr_r

...
def rotation_3d(omega, theta):
    ...
    I = np.identity(3)
    sineterm = np.sin(np.linalg.norm(omega)*theta)
    sinecoef = omega_hat/np.linalg.norm(omega)
    cosineterm = np.cos(np.linalg.norm(omega)*theta)
    coscoef = (np.dot(omega_hat,omega_hat))/(np.linalg.norm(omega)**2)
    R = I + sinecoef*sineterm + coscoef*(1-cosineterm)
    return R
...
def homog_3d(xi, theta):
    ...
    mag_omega = np.linalg.norm(xi[3:5])
    if mag_omega == 0:
        #g = np.array([np.identity(3), xi[0:3]*theta], [0, 0, 0, 1])
        g_b = np.hstack([np.identity(3), xi[0:3]*theta])
        g = np.vstack(g_b,[0, 0, 0, 1])
    else:
        omegaa1 = np.array([xi[3],xi[4],xi[5]])
        R = rotation_3d(omegaa1, theta)
        omegaa = np.array([[xi[3]], [xi[4]], [xi[5]]])
        #print(np.shape(omegaa))
        omega_hat = skew_3d(omegaa1)
        mag_term = 1/((np.linalg.norm(omegaa1))*(np.linalg.norm(omegaa1)))
        last_term = np.dot(omegaa,omegaa.T)
        #print(omegaa.T)
        #print(np.shape(omegaa.T))
        #print(last_term)
        #print(mag_term)
        v = xi[0:3]
        v = np.reshape(v,(3,1))
        #print(v)
        x = np.dot(omega_hat,v)
        #print(x)
        t = np.subtract(np.identity(3),R)
        #print(t)
        fi_term = np.dot(t, x)
        #print(fi_term)
        penul_term = np.dot(last_term,(v*theta))
        #print(penul_term)
        #print(np.shape(penul_term))
        sec_entry = mag_term*(fi_term + penul_term)
        #print(np.shape(sec_entry))
        sec_entry = np.reshape(sec_entry,(3,1))
        #print(np.shape(sec_entry))
        g_1 = np.hstack([R,sec_entry])
        #print(g_1)
...
def prod_exp(xi, theta):
    ...
    #"""""
    for n in range(pre):
        #prev = np.reshape(prev,(6,1))
        #print(np.shape(prev))
        in1 = xi[:,n]
        in2 = theta[n]
        #print(in1)
        #print(in2)
        prev = homog_3d(in1,in2)
        #print(prev)
        g = np.dot(g,prev)
